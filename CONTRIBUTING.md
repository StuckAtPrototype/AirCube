...
- Firmware releases are generated by `semantic-release` from Conventional Commits.
- Any push builds firmware and computes the next semantic version in CI.
- If commits since the latest tag require a release, CI publishes a GitHub release and uploads firmware artifacts.
- Pushes to `master` produce stable tags (`v<semver>`). Other branches publish `-dev` prereleases.

The firmware version embedded in the binary is injected by CI as `PROJECT_VER` using `semantic-release` version resolution.
